...
/-- Try one (μ, ν, c) combination for the final sesq-sum contradiction. -/
  ...
  `(tactic| (
    refine pauli_eigvec_sesq_sum_ne $μ $ν
        (by first
          | decide
          | (intro h; cases h)
          | simp [pauliOf, sigmaX, sigmaY, sigmaZ, Matrix.ext_iff, Fin.forall_fin_two]
          | (intro h
             have h00 := congrFun (congrFun h 0) 0
             have h01 := congrFun (congrFun h 0) 1
             have h11 := congrFun (congrFun h 1) 1
             simp [pauliOf, sigmaX, sigmaY, sigmaZ] at h00 h01 h11))
        (by first
          | decide
          | (constructor <;> decide)
          | (refine ⟨?_, ?_, ?_⟩ <;> decide)
          | (ext i j <;> fin_cases i <;> fin_cases j <;>
              simp [pauliOf, sigmaX, sigmaY, sigmaZ, Matrix.mul_apply, Matrix.neg_apply,
        ...
        ?_
    simp only [sesq, Kmat, pauliOf, sigmaX, sigmaY, sigmaZ, Matrix.vecMul,
      ...
      smul_eq_mul, Matrix.smul_apply, Matrix.one_fin_two, Pi.star_apply, Matrix.star_apply,
      starRingEnd_apply, star_add,
      star_mul', star_neg, star_one, star_zero, star_star, star_I_eq, mul_zero, zero_mul,
      mul_one, one_mul, add_zero, zero_add, mul_neg, neg_mul, neg_neg, neg_zero]
    all_goals (
    ...
    have hor02 : xRow U i₀ 0 * star (xRow U i₂ 0) + xRow U i₀ 1 * star (xRow U i₂ 1)
      ...
      first
        ...
        | (simp only [Matrix.mul_apply, Fin.sum_univ_four, Matrix.star_apply] at hor02
           exact hor02)
    rw [hy₀, hy₂] at hor02
    simp only [Kmat, pauliOf, sigmaX, sigmaY, sigmaZ, Matrix.vecMul,
      Matrix.mulVec, Matrix.mul_apply, dotProduct, Fin.sum_univ_two, Matrix.of_apply,
      Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.head_cons, Pi.smul_apply,
      smul_eq_mul, Matrix.smul_apply, Matrix.one_fin_two, Pi.star_apply, Matrix.star_apply,
      starRingEnd_apply, star_add, star_mul', star_neg,
      star_one, star_zero, star_star, star_I_eq, mul_zero,
      zero_mul, mul_one, one_mul, add_zero, zero_add, mul_neg, neg_mul, neg_neg,
      neg_zero] at hor02
    have hor02c := congrArg (star : ℂ → ℂ) hor02
    simp only [star_add, star_mul', star_neg, star_one, star_zero, star_star, star_I_eq,
      starRingEnd_apply, mul_neg, neg_mul, neg_neg, mul_one, one_mul] at hor02c
    first
      | (have hPv : P = BellLabel.phiM := by rw [← hP] <;> decide
         subst hPv
         first
           | bell_try BellLabel.phiM, BellLabel.psiP
           | bell_try BellLabel.phiM, BellLabel.psiM)
      | (have hPv : P = BellLabel.psiP := by rw [← hP] <;> decide
         subst hPv
         first
           | bell_try BellLabel.psiP, BellLabel.phiM
           | bell_try BellLabel.psiP, BellLabel.psiM)
      | (have hPv : P = BellLabel.psiM := by rw [← hP] <;> decide
         subst hPv
         first
           | bell_try BellLabel.psiM, BellLabel.phiM
           | bell_try BellLabel.psiM, BellLabel.psiP)))
...
theorem case_fourCycle (U : Matrix (Fin 4) (Fin 4) ℂ)
  ...
  -- eigenvectors of pauliOf P; hor02 decomposes as sesq w₀ w₂ + c * sesq w₀ ((pauliOf ν).mulVec w₂)
