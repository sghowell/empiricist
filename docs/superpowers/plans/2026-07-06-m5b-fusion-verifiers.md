...
- A **fusion** on qubits (a, b) = destructive measurement of the commuting pair **`{X_a Z_b, Z_a X_b}`** (Löbl's canonical type-II convention — AMENDED from the original `{XX,ZZ}` text by the Task-1 implementation finding, ratified in spec D6: the two differ by a free Hadamard on one fusion qubit, so F(G) is provably identical, and `{XZ,ZX}` is the convention that realizes the goldens below / the literature's graph rule). v0 postselects the (+1, +1) branch WLOG — all outcomes differ by Pauli corrections, which are local Cliffords, and our identity is the LC orbit. If the +1 branch of an observable is impossible (already-determined −1, can happen intra-component), postselect the −1 branch instead — same LC orbit.
