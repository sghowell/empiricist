...
| *status 2026-09-09* | *P6 science under the `zx` pack (M25a–M26a, PRs #85–#88 and the M26a PR): six groundwork and 29 campaign claims under `p6-zx-v1`; the campaign's best candidate (39 rules, locally confluent at depth 5 over 6,521 critical pairs) was refuted on a zero-map pair, which exposed that v1's completeness bar — one normal form per class, the zero class included — is unpassable by any local rule system. `p6-zx-v2` (zero recognised, not normalised) frozen; `zx_completeness` gained the mode with every v1 evidence file re-verifying unchanged; the six serious candidates re-baselined (all still incomplete, now with non-zero witnesses: the copy rule in Hadamard-edge form); four groundwork rows including a v1/v2 pair on one system. See `docs/science/2026-09-09-p6-zx-v2.md`.* | *`claims check` green here at 132 claims; campaign spend so far $16.24 recorded, about $88 with M26c's conservative estimate for calls killed at the timeout (true figure probably $50–65), of the $100 authorised; resumable under v2* |
