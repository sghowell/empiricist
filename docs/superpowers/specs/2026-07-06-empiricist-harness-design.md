...
| D6 | **Fusion semantics** | Fusion = **destructive Bell measurement `{X_A X_B, Z_A Z_B}`** of one qubit from each of two components **or intra-component**, realized as a **stabilizer measurement on the tableau/GF(2) rep** (never a graph-rewrite shortcut). `F` = min **successful (postselected)** fusions. **No leaf-only restriction** (the tableau handles general fusion exactly; restricting to leaves risks `F_leaf > F_true`). | Matches the P5 definition exactly; intra-component fusions are required to build cycles; postselection is WLOG (all four outcomes LC-equivalent). |
